...
def OECD_query(series: str,
    ...
    potential_time_cols: list or None
        Potential time columns to use. Defaults to ["TIME", "YEAR"] (hierarchy to choose: left first)
    potential_data_cols: list or None
        Potential data columns to use. Defaults to ["SUBJECT", "MEASURE", "INDICATOR"]
    ...
    TODO
        - figure out if we need to add more cols; problem with overlapping column names (subjects w same names)
        - Add variable for country name instead of using "LOCATION" directly
        - Add save/load functionality
    """
    # step 0: initialisation
    # step 0.1: checks on structure
    assert isinstance(series, str), "Invalid type for series, must be a string."
    assert isinstance(subject, str) or isinstance(subject, list), "Invalid type for subject, must be a string or list."
    assert isinstance(country, str) or isinstance(country, list), "Invalid type for country, must be a string or list."
    assert isinstance(measure, str) or isinstance(measure, list) or measure is None, \
    ...
    assert isinstance(extra_args, str) or extra_args is None, "Invalid type for extra_args, must be a string or None."
    assert potential_time_cols is None or isinstance(potential_time_cols, list), "Invalid type for potential_time_cols."
    assert potential_data_cols is None or isinstance(potential_data_cols, list), "Invalid type for potential_time_cols."
    assert columns_prefix is None or isinstance(columns_prefix, str), "Invalid type for columns_prefix."

    # step 0.2: set default time and data column names
    if potential_time_cols is None:
        potential_time_cols = ["TIME", "YEAR"]  # hierarchy to choose time axis from: left first (choose only one)
    if potential_data_cols is None:
        potential_data_cols = ["SUBJECT", "MEASURE", "INDICATOR"]  # choose all available
    if columns_prefix is None:
        columns_prefix = ""

    # step 1: parse inputs
    dict_input = {"subject": subject,
                  "country": country,
                  "measure": measure,
                  "frequency": frequency,
    ...
    for input_var in ["subject", "country", "measure", "frequency"]:
        ...
        elif isinstance(dict_input[input_var], list):  # else the input has to be overwritten
    ...
    df = pd.read_csv(f'https://stats.oecd.org/SDMX-JSON/data/{series}/{dict_input["subject"]}'
    ...
    df = pd.DataFrame({time_col: df[time_col].unique()})  # create new df; take the longest possible time period
...
# ---------------------------------------------------------------- #
def key_shortterm_indicators_summary(countries: Union[list, str],
                                     frequency: str = "M",
    ...
    """
    if exclude_existing_mf_subjects:  # exclude data available in other macroeconomic features class
...
def consumer_price_indices_all(countries: Union[list, str]):
    ...
    """
    return OECD_query(series="PRICES_CPI",
                      subject=countries,
